...
class Spider(BaseSpider):
    def __init__(self):
        self.name = "二小"
        self.host = "https://www.2xiaopan.top"
        self.headers = {
            "User-Agent": (
        ...
        self.categories = [
            {"type_id": "1", "type_name": "二小电影"},
            {"type_id": "2", "type_name": "二小剧集"},
            {"type_id": "3", "type_name": "二小动漫"},
            {"type_id": "17", "type_name": "二小综艺"},
        ]
        self.pan_patterns = [
    ...
    def _build_pan_lines(self, detail):
        ...
        for url in detail.get("pan_urls", []):
            ...
            if not pan_type or url in seen:
                continue
            seen.add(url)
            lines.append((self.pan_priority.get(pan_type, 999), f"{pan_type}#二小", f"{title}${url}"))
